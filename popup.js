...
function updateUI(name, category, viewers, title, type){
  ...
  if(document.getElementsByClassName(category).length == 0){         
    ...
    /* if the category exists already do
    check also if the stream isn't already added  */
...
chrome.storage.local.get(['liveChannels'], function(result) {
  ...
  // sorting the by viewer count descending (high -> low)
...
window.onload = () => {
  ...
  jQuery('.name').click(function(){
    ...
    window.open(jQuery(this).attr("data-url"), '_blank', windowsOptions);
    window.close();
  });
}